...
> **실제 구현:** 1인 개발 규모를 고려해 Statistics·Admin은 독립 서비스로 만들지 않고,
> 통계 API는 Notification Service에, 회원 관리는 Auth Service에 통합했다.
> 최종 서비스는 7개(API Gateway·Auth·Subscription·Notification·Alert Collector·Alert Processor·Frontend)다.
...
> **실제 구현:** 구독 등록 Saga는 최종 **스킵**했다. `@Transactional` + Transactional Outbox 조합으로
> 원자성이 보장되고, 구독 플로우가 분산 보상 트랜잭션을 필요로 할 만큼 복잡하지 않다고 판단했다.
> Saga 토픽(`saga.subscription` 등)도 미생성. (WBS Phase 3-A 스킵 결정 참조)
...
> **실제 구현:** Statistics Service와 `stats_db` Read Model을 만들지 않아 **CQRS는 미적용**됐다.
> 통계는 Notification Service가 `notification_history` 테이블을 요청 시점에 직접 집계해 제공한다.
> Write 경로(Outbox → Kafka → Event Store)만 구현됐고, 데이터 규모상 사전 집계 Read Model의 실익이 작다고 판단했다.
