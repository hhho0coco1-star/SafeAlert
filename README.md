...
> 두 가지 실행 경로를 제공합니다.
> - **로컬 개발 (아래 섹션)**: docker-compose(인프라) + `gradlew bootRun`(서비스) — 빠른 개발·디버깅용
> - **Kubernetes 배포**: minikube 전체 배포 — 운영 환경에 가까운 구성 검증용 (하단 "Kubernetes 배포 (minikube)" 참고)
...
| Phase | 내용 | 상태 |
|-------|------|------|
| Phase 0 | Docker + K8s 인프라 구성 (Kafka, Redis, PostgreSQL, MongoDB) | ✅ |
| Phase 1 | Auth·Subscription·Notification·Alert 서비스, React 프론트엔드, OAuth2, 이메일 인증, 공공 API 파이프라인, 시/군/구 구독 시스템 | ✅ |
| Phase 2 | 이벤트 파이프라인 (Alert Collector → Kafka → Processor → Notification → WebSocket) | ✅ |
